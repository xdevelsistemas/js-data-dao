...
class ForgotController {
    ...
    sendMail(req, res, next) {
        return this.forgot.sendForgotMail(req.body).then(() => {
            res.status(200);
    ...
    validaToken(req, res, next) {
        return this.forgot.validaToken(req.params).then(dados => {
            res.status(200);
            return dados;
        }).catch(err => {
    ...
    resetPassword(req, res, next) {
        return this.forgot.resetPassword(req.params, req.body).then(dados => {
            res.status(200);
            return dados;
        }).catch(err => {
            throw new services_1.APIError(err, 401);
